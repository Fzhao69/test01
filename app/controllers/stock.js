...
var StockController = {
    showStocManagekList: function(req, res, next) {
        ...
        if (req.query.keyValue != '') {
            data.$or = [{ name: { $regex: req.query.keyValue } }, { code: { $regex: req.query.keyValue } }];
        }
    ...
    stockManageAdd: function(req, res, next) {
        ...
        stocksModel.count({ name: stock.name, code: stock.code })
            .then(function(result) {
                if (result > 0) {
                    ...
                    return next({ res: { error: 1, error_message: "商品：" + stock.name + '&emsp;编码：' + stock.code + "&emsp;已存在", data: null } })
                }
                next();
            })
            .catch(function(e) {
    ...
    stockManageEdit: function(req, res, next) {
        ...
        stock.selling_price = stock.selling_price * 100;
        delete stock.amount;
        stocksModel.count({ name: stock.name, code: stock.code, _id: { $ne: stock._id } })
            .then(function(result) {
                if (result > 0) {
                    logModel.find({}).sort({ 'time': -1 }).limit(1)
                        .then(function(re) {
    ...
    stockManageMinus: function(req, res, next) {
        ...
        stocksModel.find({ _id: stock._id })
            .then(function(result) {
                ...
                if (surplus < stock.surplus) {
                    ...
                    return next({ res: { error: 1, error_message: "商品库存不足", data: null } })
                }
    ...
    stockManageSave: function(req, res, next) {
        ...
        if (_id.length < 1) {
            ...
            stocksModel.update({ _id: _id }, stock)
                .then(function(result) {
                    next({ res: { data: result } })
                })
                .catch(function(e) {
    ...
    stockManageDelete: function(req, res, next) {
        stocksModel.remove({ _id: { $in: req.body._ids } })
            .then(function(result) {
    ...
    showStockList: function(req, res, next) {
        ...
        var param = req.query;
        var data = {};
        data.path = { $regex: param.selectValue }
        // console.log(param)
        ...
        if (param.keyValue != '') {
            data.$or = [{ commodity: { $regex: param.keyValue } }, { user: { $regex: param.keyValue } }];
        }
        // console.log(data)
        logModel.find(data).sort({'time':-1})
            .then(function(result) {
                var data = {
                    draw: 1,
                    recordsTotal: result.length,
